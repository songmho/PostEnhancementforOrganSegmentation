...
class MedImageEnhancer:
        ...
        if case == 1:    # Case SEQ_np_(i-1) violates appearance principle
            ...
            if len(revised_seq["data"]) > 0:  # if any sequence in the cur_ref_seqs
                ...
                for i in range(len(seq_np_cur["data"])):  # To repeat whole slices in false sequence
                    ...
                    cur_seg = self.__revise_slsegs_violating_appearance(sl_seg_prv_last, cur_seg, None,
                                                                                last_img, cur_img,
                ...
                for i in range(len(seq_np_cur["data"])-1, -1,
                    ...
                    cur_seg = seq_np_cur["data"][i]["img"]
                    cur_img = self.srs_org_sl[seq_np_cur["data"][i]["id"]]["img"]
                    cur_seg = self.__revise_slsegs_violating_appearance(None, cur_seg, sl_seg_cur_first,
                                                                                 None, cur_img,
            ...
            if case == 3:
                ...
                for i in range(len(seq_np_cur["data"]) - 1, 0):
                    ...
                    seq_np_cur["data"][i]["img"] = self.__revise_slsegs_violating_appearance(None, sl_seg_cur,
    ...
    def detect_location_inconsistency(self, id):
        nxt_id = self.__get_maximum_size_sl_id(self.sequences[id]["data"])
        # print("  SEQUENCE # ", id,"  ", nxt_id)
        self.process_statistics["location"]["remedy_states"][
            self.sequences[id]["data"][nxt_id]["id"]] = "Not Violated"
        self.revise_location_inconsistency(id, nxt_id=nxt_id)

        prv_id = self.__get_maximum_size_sl_id(self.sequences[id]["data"])
        # print("  SEQUENCE # ", id,"  ", prv_id)
        self.revise_location_inconsistency(id, prv_id=prv_id)

    def revise_location_inconsistency(self, id, nxt_id=None, prv_id=None):
        if nxt_id is not None:
            # print("NXT_ID", nxt_id)
            range_loop = range(nxt_id-1, -1, -1)
            list_sl_seg_sections = self.__divide_sections(self.sequences[id]["data"][nxt_id]["img"])
            trg_id = nxt_id
        else:
            # print("PRV_ID", prv_id)
            range_loop = range(prv_id + 1, len(self.sequences[id]["data"]))
            list_sl_seg_sections = self.__divide_sections(self.sequences[id]["data"][prv_id]["img"])
            trg_id = prv_id
        slseg_trg = self.sequences[id]["data"][trg_id]["img"]
        sl_trg = self.__get_current_sl(id, trg_id)
        for sl_id in range_loop:  # Max to first
            self.process_statistics["location"]["remedy_states"][self.sequences[id]["data"][sl_id]["id"]] = "Not Violated"
            list_sl_seg_sections_cur = self.__divide_sections(self.sequences[id]["data"][sl_id]["img"])
            ...
            for sec_cur in list_sl_seg_sections_cur:
                is_contained = False
                idx = -1
                for sec_trg in list_sl_seg_sections:
                    idx+=1
            ...
            for sec_trg_idx in list_not_contained_secs:
                ...
                if need_to_connect:
                    ...
                    if prv_id is None:
                        ...
                        new_sec = self.__revise_slsegs_violating_appearance(None, new_sec, sec_trg, None, sl_cur, sl_trg, 0.8)
            ...
            list_sl_seg_sections = self.__divide_sections(self.sequences[id]["data"][sl_id]["img"])
            sl_trg = sl_cur
            slseg_trg = slseg_cur
    ...
    def detect_size_inconsistency(self, id):
        ...
        # print(">>>> SEQ: ",id, "    ", self.sequences[id]["data"][max_id]["fname"])
        for sl_id in range(1, len(self.sequences[id]["data"])):
            # print(self.sequences[id]["data"][sl_id]["fname"], self.sequences[id]["data"][0]["fname"],
            #       np.count_nonzero(self.sequences[id]["data"][sl_id]["img"]), size_sl_seg_prv)
    ...
    def revise_size_inconsistency(self, list_groups, id):
        # try:
        ...
        size_transition = self.__compute_transition(nxt_group, 0)
        ...
        max_id = self.__get_maximum_size_sl_id(self.sequences[id]["data"])
        list_nxt_sections = self.__divide_sections(self.sequences[id]["data"][max_id]["img"])
        for i in reversed(range(max_id)):
            list_cur_sections = self.__divide_sections(self.sequences[id]["data"][i]["img"])
            ...
            list_nxt_sections = self.__divide_sections(self.sequences[id]["data"][i]["img"])

        list_prv_sections = self.__divide_sections(self.sequences[id]["data"][max_id]["img"])
    ...
    def correct_shape_inconsistency(self):
        ...
        for id in range(len(self.sequences)):  # Loop for sequences
            ...
            # To find size consistency violated SLSegs
            # To make groups depending on high difference of size between adjacent SLSeg
            list_groups = self.detect_shape_inconsistency(id)
    ...
    def detect_shape_inconsistency(self, seq_id):
        list_groups = []
        sl_seg_prv = self.sequences[seq_id]["data"][0]["img"]
        sl_seg_prv_secs = self.__divide_sections(sl_seg_prv)
        ...
        for sl_id in range(1, len(self.sequences[seq_id]["data"])):
            ...
            sl_seg_cur_secs = self.__divide_sections(sl_seg_cur)
            is_satisfy = True
            ...
            if is_satisfy:
                ...
                # print(">> ", self.sequences[seq_id]["data"][sl_id]["fname"])
                list_groups.append(cur_group)
    ...
    def revise_shape_inconsistency(self, list_groups):
        max_size = 0
        max_group_idx = -1
    ...
    def detect_HU_scale_inconsistency(self, id):
        for sl_id in range(len(self.sequences[id]["data"])):
            cur_sl_org = self.__get_current_sl(id, sl_id)
            cur_slseg = self.sequences[id]["data"][sl_id]["img"]
            list_sl_seg_sections = self.__divide_sections(cur_slseg)
            ...
            for sl_seg_section in list_sl_seg_sections:
                cur_hu_scale, frequency = self.__compute_HU_scale(cur_sl_org, sl_seg_section)
                violated_area = 0
                sect_area = np.count_nonzero(sl_seg_section)
                for hu in frequency.keys():
                ...
                if violated_rate == 0.0:  # Case 0. Not Violated. If all HU scales satisfy the range
                    ...
                    self.sequences[id]["data"][sl_id]["img"] = self.revise_HU_scale_inconsistency(id, sl_id, sl_seg_section, 1)
                elif violated_rate > 0.03:  # Cases 2 and 3
                    is_contour, violated_area = self.__check_HU_scale_violated_pixel_location(cur_sl_org, sl_seg_section)
                    if is_contour:  # Case 2. If the violated pixels are located in the contour, Set False outer area
                        self.sequences[id]["data"][sl_id]["img"] = self.revise_HU_scale_inconsistency(id, sl_id, sl_seg_section, 2, violated_area)
                    else:  # Case 3
                        self.sequences[id]["data"][sl_id]["img"] = self.revise_HU_scale_inconsistency(id, sl_id, sl_seg_section, 3)
                else:  # Cases 4 and 5
                    is_contour, violated_area = self.__check_HU_scale_violated_pixel_location(cur_sl_org, sl_seg_section)
                    if is_contour:  # Case 5. If the violated pixels are located in the contour, Set False outer area
                        self.sequences[id]["data"][sl_id]["img"] = self.revise_HU_scale_inconsistency(id, sl_id, sl_seg_section, 5, violated_area)
                    else:  # Case 4. If the violated pixels are located in the middle, ignore
                        self.sequences[id]["data"][sl_id]["img"] = self.revise_HU_scale_inconsistency(id, sl_id, sl_seg_section, 4)
    ...
    def revise_HU_scale_inconsistency(self, seq_id, sl_id, sl_seg_sec, violation_case, violated_area=None):
        result = np.zeros(sl_seg_sec.shape, np.uint8)
        state = "Remedied"
        if violation_case in [1, 3]:
             result = cv2.subtract(self.sequences[seq_id]["data"][sl_id]["img"], sl_seg_sec) # Discard the section from SLSeg
        elif violation_case in [2, 5]:
            if np.count_nonzero(np.bitwise_and(violated_area, self.sequences[seq_id]["data"][sl_id]["img"]))==0:
                state = "Not Violated"
            result = np.array(np.where(violated_area > 0, 0, self.sequences[seq_id]["data"][sl_id]["img"]), np.uint8)
        elif violation_case == 4:
            result = sl_seg_sec
            state = "Not Violated"
    ...
    def __revise_slseg_violating_shape(self, msk_a, msk_b, sl_org):
    ...
    def __revise_slsegs_violating_appearance(self, prv_seg, cur_seg, nxt_seg, prv_img, cur_img, nxt_img, size_diff_trend=1.0):
        """
    ...
    def __compute_HU_scale(self, sl_org, sl_seg_sec):
        try:
            hu_values = np.where(sl_seg_sec > 0, sl_org, np.inf)
            hu_values = hu_values[hu_values != np.inf]
...
if __name__ == '__main__':
    ...
    #         pe.correct_appearance_inconsistency()
    ...
    #         pe.correct_shape_inconsistency()
    ...
    #         pe.correct_HU_scale_inconsistency()
