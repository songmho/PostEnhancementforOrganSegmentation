...
class ImageConverter:
    ...
    def convert_slice(self):
        ...
        if self.format == "DICOM":
            for key, sl in self.org_dcm.items():
                ...
                # print(rescale_slope, rescale_intercept, ww, wc)
                #
                #
                # img = rescale_slope*img + rescale_intercept
                # ymin = 0
    ...
    def convert_mask(self):
        ...
        for i in range(self.org_nii.header.get_data_shape()[2]):
            ...
            img = np.array(list(zip(*img))[::-1])
            img = np.array(np.where((img >0) & (img==1), 255, 0), dtype=np.uint8)
...
if __name__ == '__main__':
    ic = ImageConverter("DICOM")
    path_srs = r"D:\Dataset\LLU Dataset\7083077_10302013\01. Original CT Study\01. DICOM\tt"
    path_save_srs = r"E:\1. Lab\Daily Results\2022\2203\0314\rrrrss"
